...
public class UsuarioController {
    ...
    public ResponseEntity<List<Usuario>> getAll() {
        ...
        try (Connection connection = conexao.getConexao();
            ...
            while (rs.next()) {
                ...
                usuarioEncontrado.setDataNascimento(rs.getObject("DATANASCIMENTO", LocalDate.class));
    ...
    public ResponseEntity<String> editUsuario(@PathVariable Long id, @RequestBody Usuario usuario) {
        String querySQL = "UPDATE USUARIO SET NOME=?, EMAIL=?, GENERO=?, PAIS=?, DATANASCIMENTO=? WHERE ID=?";
        try (Connection connection = conexao.getConexao();
             PreparedStatement ps = connection.prepareStatement(querySQL)) {
            ...
            ps.setObject(5, usuario.getDataNascimento());
            ps.setLong(6, id);
    ...
    public ResponseEntity<String> createUser(@RequestBody Usuario usuario) {
        String querySQL = "INSERT INTO USUARIO (NOME, EMAIL, GENERO, PAIS, DATANASCIMENTO) VALUES (?, ?, ?, ?, ?)";
        try (Connection connection = conexao.getConexao();
             PreparedStatement ps = connection.prepareStatement(querySQL)) {
            ps.setString(1, usuario.getNome());
            ps.setString(2, usuario.getEmail());
